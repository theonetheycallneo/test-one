...
var import_config = require("./config"), import_matchers = require("./matchers"), import_getPageExport = require("./utils/getPageExport"), validPlatforms = /* @__PURE__ */ new Set([
  "android",
  "ios",
  "native",
  "web"
  ...
  try {
    for (var _loop = function() {
      ...
      if (meta.specificity < 0)
        return "continue";
      var type = meta.isLayout ? "layout" : meta.renderMode || (0, import_config.getDefaultRenderMode)(), node = {
        type,
        loadRoute() {
...
function getFileMeta(key, options) {
  key = key.replace(/^\.\//, "");
  var parts = key.split("/"), route = (0, import_matchers.removeSupportedExtensions)(key), filename = parts[parts.length - 1], filenameWithoutExtensions = (0, import_matchers.removeSupportedExtensions)(filename), isLayout = filenameWithoutExtensions.startsWith("_layout"), [_fullname, renderModeFound] = filename.match(/\+(api|ssg|ssr|spa)\.(\w+\.)?[jt]sx?$/) || [], renderMode = renderModeFound;
...
function getIgnoreList(options) {
  var _options_ignore, ignore = [
    /^\.\/\+html\.[tj]sx?$/,
    ...(_options_ignore = options == null ? void 0 : options.ignore) !== null && _options_ignore !== void 0 ? _options_ignore : []
  ];
  return (options == null ? void 0 : options.preserveApiRoutes) !== !0 && ignore.push(/\+api\.[tj]sx?$/), ignore;
}
function extrapolateGroups(key) {
  var keys = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : /* @__PURE__ */ new Set(), match = (0, import_matchers.matchArrayGroupName)(key);
  if (!match)
...
function generateDynamic(path) {
  var dynamic = path.split("/").map(function(part) {
    ...
    var deepDynamicName = (0, import_matchers.matchDeepDynamicRouteName)(part), dynamicName = deepDynamicName ?? (0, import_matchers.matchDynamicName)(part);
    return dynamicName ? {
      name: dynamicName,
...
function getLayoutNode(node, options) {
  var groupName = (0, import_matchers.matchGroupName)(node.route), childMatchingGroup = node.children.find(function(child) {
    return child.route.replace(/\/index$/, "") === groupName;
  }), initialRouteName = childMatchingGroup == null ? void 0 : childMatchingGroup.route;
  return {
    ...node,
...
function crawlAndAppendInitialRoutesAndEntryFiles(node, options) {
  ...
  if (node.type === "spa" || node.type === "ssg" || node.type === "ssr")
    ...
    var groupName = (0, import_matchers.matchGroupName)(node.route), childMatchingGroup = node.children.find(function(child2) {
