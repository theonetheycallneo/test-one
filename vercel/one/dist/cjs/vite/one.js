...
function one(options = {}) {
  ...
  const vxrnOptions = (0, import_vxrn.getOptionsFilled)(), root = vxrnOptions?.root || process.cwd(), { clientEnvDefine } = (0, import_vxrn.loadEnv)(vxrnOptions?.mode ?? "development"), devAndProdPlugins = [
    {
      name: "one-define-env",
      config() {
        return {
    ...
    new Proxy(
      {
        ...
        config(configIncoming) {
          ...
          pathsConfig !== !1 && (configIncoming.plugins?.flat().some((p) => p && p.name === "vite-tsconfig-paths") || (tsConfigPathsPlugin = (0, import_vite_tsconfig_paths.default)(
