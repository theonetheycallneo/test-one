...
function one(options = {}) {
  ...
  const { optimizeDeps } = getOptimizeDeps("build"), optimizeIds = optimizeDeps.include, optimizeIdRegex = new RegExp(
  ...
  const vxrnOptions = getOptionsFilled(), root = vxrnOptions?.root || process.cwd(), { clientEnvDefine } = loadEnv(vxrnOptions?.mode ?? "development"), devAndProdPlugins = [
    {
      name: "one-define-env",
      config() {
        return {
    ...
    new Proxy(
      {
        ...
        config(configIncoming) {
          ...
          pathsConfig !== !1 && (configIncoming.plugins?.flat().some((p) => p && p.name === "vite-tsconfig-paths") || (tsConfigPathsPlugin = tsconfigPaths(
  ...
  return globalThis.__vxrnAddNativePlugins = [clientTreeShakePlugin()], globalThis.__vxrnAddWebPluginsProd = devAndProdPlugins, [
    ...devAndProdPlugins,
    {
      name: "one:react-scan",
      config() {
