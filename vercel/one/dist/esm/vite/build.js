...
async function build(args) {
  labelProcess("build");
  const userOptions = await loadUserOneOptions("build"), serverOutputFormat = userOptions.build?.server?.outputFormat ?? "esm";
  ...
  const vxrnOutput = await vxrnBuild(
    ...
    args
  ), options = await fillOptions(vxrnOutput.options), toAbsolute = (p) => Path.resolve(options.root, p), manifest = getManifest(), { optimizeDeps } = getOptimizeDeps("build"), apiBuildConfig = mergeConfig(
...
`);
    ...
    ), apiRouteExternalRegex = buildRegexExcludingDeps(optimizeDeps.include), apiEntryPoints = manifest.apiRoutes.reduce((entries, { page, file }) => (entries[page.slice(1) + ".js"] = join("app", file), entries), {}), apiOutputFormat = userOptions?.build?.api?.outputFormat ?? serverOutputFormat, treeshake = userOptions?.build?.api?.treeshake;
    await viteBuild(
            ...
            output: {
              ...
              ...apiOutputFormat === "esm" ? {
                ...
                chunkFileNames: (chunkInfo) => {
                  const dir = Path.dirname(chunkInfo.name), name = Path.basename(chunkInfo.name, Path.extname(chunkInfo.name));
                  return Path.join(dir, `${name}-[hash].cjs`);
                },
                assetFileNames: (assetInfo) => {
                  const name = assetInfo.name ?? "", dir = Path.dirname(name), baseName = Path.basename(name, Path.extname(name)), ext = Path.extname(name);
...
`);
  ...
  const staticDir = join("dist/static"), clientDir = join("dist/client");
  ...
  for (const [index, output] of outputEntries) {
    ...
    const id = output.facadeModuleId || "", file = Path.basename(id);
    if (!id || file[0] === "_" || file.includes("entry-server") || id.includes("+api") || !id.includes("/app/"))
      continue;
    const relativeId = relative(process.cwd(), id).replace("app/", "/"), onlyBuild = vxrnOutput.buildArgs?.only;
...
`), process.env.DEBUG && console.info("paramsList", JSON.stringify(paramsList, null, 2));
    for (const params of paramsList) {
      const cleanId = relativeId.replace(/\+(spa|ssg|ssr)\.tsx?$/, ""), path = getPathnameFromFilePath(cleanId, params, foundRoute.type === "ssg"), htmlPath = `${path.endsWith("/") ? `${removeTrailingSlash(path)}/index` : path}.html`, clientJsPath = join("dist/client", clientManifestEntry.file), htmlOutPath = toAbsolute(join(staticDir, htmlPath));
      let loaderData = {};
      try {
        console.info(`  \u21A6 route ${path}`);
        const cleanPath = path === "/" ? path : removeTrailingSlash(path), preloadPath = getPreloadPath(path);
        if (await FSExtra.writeFile(
...
`)
          ...
          if (globalThis.__vxrnLoaderProps__ = loaderProps, globalThis.__vxrnresetState?.(), exported.loader) {
            ...
            const code = await readFile(clientJsPath, "utf-8"), withLoader = replaceLoader({
              code,
              loaderData
...
${JSON.stringify(params || null, null, 2)}`
  ...
  let postBuildLogs = [];
  (userOptions.web?.deploy ?? options.server?.platform) === "vercel" && (await FSExtra.writeFile(
...
async function moveAllFiles(src, dest) {
  try {
    await FSExtra.copy(src, dest, { overwrite: !0, errorOnExist: !1 });
  } catch (err) {
    console.error("Error moving files:", err);
  }
}
function getPathnameFromFilePath(path, params = {}, strict = !1) {
  const dirname = Path.dirname(path).replace(/\([^\/]+\)/gi, ""), fileName = Path.basename(path).replace(/\.[a-z]+$/, ""), nameWithParams = (() => {
    if (fileName === "index")
      return "/";
