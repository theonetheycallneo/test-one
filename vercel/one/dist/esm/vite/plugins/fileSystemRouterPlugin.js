    ...
    return createHandleRequest(options, {
      async handleSSR({ route, url, loaderProps }) {
        ...
        try {
          ...
          const entry = await runner.import(virtualEntryId), render = entry.default.render;
...
`), `
      ...
      async handleAPI({ request, route: route2, url: url2, loaderProps: loaderProps2 }) {
        return await resolveAPIRequest(
          () => runner.import(join("app", route2.file)),
          request,
          loaderProps2?.params || {}
        );
      }
  ...
  return {
            ...
            entries: [
              ...new Set(
                handleRequest.manifest.pageRoutes.flatMap((route2) => [
                  ...
                  ...route2.layouts?.map((layout) => join("app", layout.contextKey)) || []
                ])
              )
            ]
          }
    ...
    configureServer(serverIn) {
      server = serverIn, runner = createServerModuleRunner(
        USE_SERVER_ENV ? server.environments.server : server.environments.ssr
      );
      const appDir = join(process.cwd(), "app"), fileWatcherChangeListener = debounce(async (type, path) => {
