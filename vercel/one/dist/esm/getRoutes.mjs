import { getDefaultRenderMode } from "./config.mjs";
import { matchArrayGroupName, matchDeepDynamicRouteName, matchDynamicName, matchGroupName, removeSupportedExtensions } from "./matchers.mjs";
...
function getDirectoryTree(contextModule, options) {
  ...
  for (const filePath of contextModule.keys()) {
    ...
    if (!(process.env.NODE_ENV === "development" && node.type !== "api" && importMode === "sync" && !getPageExport(node.loadRoute()))) for (const route of extrapolateGroups(meta.route)) {
